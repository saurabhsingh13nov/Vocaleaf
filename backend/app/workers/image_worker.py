...
async def _load_page_and_job(
    ...
    *,
    ...
    page_result = await db.execute(
        ...
        .options(selectinload(StoryPage.story))
...
async def run_image_generation_in_session(
    ...
    *,
    ...
    try:
        result = get_google_imagen_client().generate_image(**request_payload)
    except GoogleImagenError as exc:
        await mark_image_generation_failed_in_session(
            db,
            story_page_id=story_page_id,
            job_id=job_id,
    ...
    object_key = (
        ...
        f"/pages/{page.page_number}/image/{image_uuid}.png"
    )
    ...
    # Check if all pages for this story now have images → mark story ready
    ...
    all_images_done = all(p.status == StoryPageStatus.IMAGE_READY for p in all_pages)
